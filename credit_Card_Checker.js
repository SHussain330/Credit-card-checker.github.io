...
const findInvalidCards = cardsArray => {
  ...
  for(let i = 0 ; i < cardsArray.length ; i++){
    let currentInvalidCard = cardsArray[i];
    if(!validateCred(currentInvalidCard)){
      invalidCardsArray.push(currentInvalidCard);
    }
  }
...
const idInvalidCardCompanies = batchArray => {
  ...
  for(let i = 0 ; i < batchArray.length ; i++ ){
    if(batchArray[i][0]){
      ...
      else if (arrayOfCompanies.indexOf('Visa') === -1) {
          arrayOfCompanies.push('Visa');
      }
      else if (arrayOfCompanies.indexOf('Mastercard') === -1) {
         arrayOfCompanies.push('Mastercard');
        }
      else if (arrayOfCompanies.indexOf('Discover') === -1) {
